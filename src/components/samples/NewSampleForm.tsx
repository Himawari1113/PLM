...
import { ArrowLeft, Box, ChevronLeft, ChevronRight, DollarSign, Factory, FileText, FlaskConical, ImagePlus, Package, Palette, Plus, Scissors, Sparkles, Star, Trash2, X } from 'lucide-react'
import { useTranslations } from 'next-intl'
import { Link, useRouter } from '@/lib/navigation'
import { SAMPLE_TYPE_LABELS } from '@/lib/constants'
import { BpPageHeader } from '@/components/common'
import { exportTechPack } from '@/lib/exportTechPack'
...
export function NewSampleForm({
  ...
  const [sampleImages, setSampleImages] = useState<Array<{ dataUrl: string; fileName: string }>>([])
  ...
  const applyAiImageAsSample = async (url: string) => {
    try {
      ...
      const reader = new FileReader()
      reader.onloadend = () => {
        setSampleImages((prev) => [...prev, { dataUrl: reader.result as string, fileName: `ai-generated-${prev.length + 1}.png` }])
        setActiveImageIndex((prev) => sampleImages.length)
      }
      reader.readAsDataURL(blob)
    } catch (err) {
      console.error('Failed to download AI image:', err)
      setSampleImages((prev) => [...prev, { dataUrl: url, fileName: `ai-generated-${prev.length + 1}.png` }])
      setActiveImageIndex(sampleImages.length)
    }
  }

  const [colorRows, setColorRows] = useState<Array<{ colorId: string; colorInput: string; status: string }>>([{ colorId: '', colorInput: '', status: 'IN_PROGRESS' }])
  ...
  useEffect(() => {
    ...
    fetch(`/api/samples/${sampleId}`)
      ...
      .then((data) => {
        ...
        if (data.imageUrl) {
          const urls = Array.isArray(data.imageUrls) ? data.imageUrls : [data.imageUrl]
          setSampleImages(urls.map((u: string, i: number) => ({ dataUrl: u, fileName: `image-${i + 1}` })))
        }
        ...
        setSalesStart(data.salesStart ? data.salesStart.split('T')[0] : '')
        setPoDeadline(data.poDeadline ? data.poDeadline.split('T')[0] : '')
        setOriginalPrice(data.originalPrice != null ? String(data.originalPrice) : '')
        setPlanQty(data.planQty != null ? String(data.planQty) : '')
        setColorRows(Array.isArray(data.sampleColors) && data.sampleColors.length > 0 ? data.sampleColors.map((x: any) => ({ colorId: x.colorId, colorInput: '', status: x.status || 'IN_PROGRESS' })) : [{ colorId: '', colorInput: '', status: 'IN_PROGRESS' }])
  ...
  useEffect(() => {
    ...
    setColorRows((prev) =>
      prev.map((row) => {
        if (row.colorId && !row.colorInput) {
          const c = colors.find((x) => x.id === row.colorId)
          return c ? { ...row, colorInput: `${c.colorCode} - ${c.colorName}` } : row
        }
        return row
      }),
    )
  ...
  const suggestedProducts = useMemo(() => {
    ...
    return products
      .filter((x) => `${x.styleNumber} ${x.name} ${x.division?.name || ''} ${x.category || ''}`.toLowerCase().includes(q))
      .slice(0, 30)
  }, [products, productInput])

  useEffect(() => {
    if (fixedProductId) return
    const exact = products.find((x) => `${x.styleNumber} - ${x.name}` === productInput)
    if (exact) setProductId(exact.id)
  }, [productInput, products, fixedProductId])
    ...
    const payload = {
      ...
      newSampleData: {
        sampleInfo: {
          ...
          imageFileName: sampleImages[thumbnailIndex]?.fileName || sampleImages[0]?.fileName || '',
          imageDataUrl: sampleImages[thumbnailIndex]?.dataUrl || sampleImages[0]?.dataUrl || '',
          thumbnailIndex,
          sampleImages,
          illustratorFileName,
          illustratorDataUrl,
  ...
  const handleExportTechPack = () => {
    const thumbnail = sampleImages[thumbnailIndex] || sampleImages[0]
    exportTechPack({
      sampleNumber: sampleNumberDisplay,
      sampleName: sampleNameEn,
      year,
      season: seasonTerm,
      division,
      subCategory,
      sampleType,
      status: statusUi,
      supplierName: selectedSupplier?.name || supplierInput,
      mainFactoryCode,
      originCountry,
      ...
      colors: colorRows
        ...
        .map((r) => {
          ...
          return { colorCode: c?.colorCode || '', colorName: c?.colorName || '', status: r.status }
        }),
      mainFabric,
      subFabrics,
      subMaterials,
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {/* Two-column layout: Main content + Communication History sidebar */}
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', minHeight: '100vh' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="bp-card" style={{ marginBottom: 24 }}>
            ...
            <div className="bp-card__content">
              <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
                {/* Left: Image + AI Design Generator */}
                <div style={{ width: 360, flexShrink: 0 }}>
                  <div className="bp-form-group">
                    {/* Label row with buttons */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <label className="bp-label" style={{ marginBottom: 0 }}>{t('sampleImage')}</label>
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            const newImages = await Promise.all(
                              files.map(async (file) => ({ dataUrl: await toDataUrl(file), fileName: file.name })),
                            )
                            setSampleImages((prev) => [...prev, ...newImages])
                            if (sampleImages.length === 0 && newImages.length > 0) setActiveImageIndex(0)
                            e.target.value = ''
                          }}
                        />
                      </label>
                    ...
                    {illustratorFileName ? <p style={{ marginTop: 4, fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>{illustratorFileName}</p> : null}
                    {/* Main preview */}
                    <div style={{ position: 'relative', marginTop: 8 }}>
                      {sampleImages.length > 0 ? (
                        <>
                          <img
                            src={sampleImages[activeImageIndex]?.dataUrl}
                            alt={sampleImages[activeImageIndex]?.fileName || 'sample-design'}
                            style={{ width: 360, height: 360, objectFit: 'cover', borderRadius: 8, border: '1px solid var(--color-border)' }}
                          />
                          <button
                            type="button"
                            onClick={() => {
                              const removing = activeImageIndex
                              setSampleImages((prev) => prev.filter((_, i) => i !== removing))
                              setActiveImageIndex((prev) => Math.max(0, Math.min(prev, sampleImages.length - 2)))
                              setThumbnailIndex((prev) => {
                                if (prev === removing) return 0
                                if (prev > removing) return prev - 1
                                return prev
                              })
                            }}
                            style={{
                        ...
                        <div
                          style={{
                            width: 360, height: 360, borderRadius: 8,
                    ...
                    {sampleImages.length > 1 && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 8 }}>
                        <button
                          ...
                          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: activeImageIndex === 0 ? 'var(--color-border)' : 'var(--color-text-secondary)' }}
                        >
                          <ChevronLeft style={{ width: 18, height: 18 }} />
                        </button>
                        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', flex: 1, paddingBlock: 2 }}>
                          {sampleImages.map((img, idx) => (
                            <div key={idx} style={{ position: 'relative', flexShrink: 0 }}>
                              ...
                              >
                                <img src={img.dataUrl} alt={img.fileName} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                              </button>
                              <button
                                type="button"
                                title={idx === thumbnailIndex ? 'List thumbnail' : 'Set as list thumbnail'}
                                onClick={(e) => { e.stopPropagation(); setThumbnailIndex(idx) }}
                                style={{
                                  position: 'absolute', top: -4, right: -4,
                                  width: 18, height: 18, borderRadius: '50%', padding: 0,
                                  background: idx === thumbnailIndex ? 'var(--color-warning, #f59e0b)' : 'rgba(255,255,255,0.85)',
                                  border: idx === thumbnailIndex ? '1.5px solid var(--color-warning, #f59e0b)' : '1px solid var(--color-border)',
                                  cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                                }}
                              >
                                <Star style={{ width: 10, height: 10, color: idx === thumbnailIndex ? '#fff' : 'var(--color-text-tertiary)', fill: idx === thumbnailIndex ? '#fff' : 'none' }} />
                              </button>
                            </div>
                          ))}
                        </div>
                        <button
                          type="button"
                          onClick={() => setActiveImageIndex((prev) => Math.min(sampleImages.length - 1, prev + 1))}
                          disabled={activeImageIndex === sampleImages.length - 1}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: activeImageIndex === sampleImages.length - 1 ? 'var(--color-border)' : 'var(--color-text-secondary)' }}
                        >
                  ...
                  {/* AI Design Generator (collapsible) */}
                  <div style={{ marginTop: 24 }}>
                    <button
                      type="button"
                      onClick={() => setAiDesignOpen((prev) => !prev)}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 6, width: '100%',
                        padding: '8px 12px', background: 'var(--color-gray-50)', borderRadius: aiDesignOpen ? '8px 8px 0 0' : 8,
                        border: '1px solid var(--color-border)', cursor: 'pointer',
                        borderBottom: aiDesignOpen ? 'none' : undefined,
                      }}
                    >
                      <Sparkles style={{ width: 16, height: 16, color: 'var(--color-primary)' }} />
                      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-gray-700)', flex: 1, textAlign: 'left' }}>AI Design Generator</span>
                      <ChevronRight style={{ width: 16, height: 16, color: 'var(--color-text-tertiary)', transition: 'transform 0.2s', transform: aiDesignOpen ? 'rotate(90deg)' : 'none' }} />
                    </button>
                    {aiDesignOpen && (
                      <div style={{ padding: 12, background: 'var(--color-gray-50)', borderRadius: '0 0 8px 8px', border: '1px solid var(--color-border)', borderTop: 'none' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 6 }}>
                              {aiImages.map((url, idx) => (
                                <button
                                  ...
                                  style={{
                                    padding: 0,
                                    border: sampleImages.some((img) => img.dataUrl === url) ? '2px solid var(--color-primary)' : '1px solid var(--color-border)',
                                    borderRadius: 6,
                                    cursor: 'pointer',
                                    background: 'none',
                                    overflow: 'hidden',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
                    <div className="bp-form-group">
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', minHeight: 42, fontSize: 18, fontWeight: 700, color: 'var(--color-text)' }}>
                    ...
                    <div className="bp-form-group">
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', minHeight: 42, fontSize: 14, color: 'var(--color-text)' }}>
                        {year || '-'}
                      </div>
                    </div>
                    <div className="bp-form-group">
                    ...
                    <div className="bp-form-group" style={{ gridColumn: '1 / -1' }}>
                      <label className="bp-label">{t('product')} (Line Plan) *</label>
                      <input
                        className="bp-input"
                        ...
                        onChange={(e) => {
                          ...
                          setProductInput(val)
                          const matched = products.find((p) => `${p.styleNumber} - ${p.name}` === val)
                          if (matched) {
                            setProductId(matched.id)
                            setDivision(matched.division?.name || '')
                            setSubCategory(matched.category || '')
                            setSalesStart(matched.salesStart ? matched.salesStart.split('T')[0] : '')
                            setOriginalPrice(matched.originalPrice != null ? String(matched.originalPrice) : '')
                            setPlanQty(matched.planQty != null ? String(matched.planQty) : '')
                            const season = matched.collection?.season
                            if (season?.year) setYear(String(season.year))
                            if (season?.term) setSeasonTerm(season.term === 'FW' ? 'FW' : 'SS')
                          } else {
                            setProductId('')
                          }
                        }}
                        placeholder="Search by style number, name, division..."
                        readOnly={Boolean(fixedProductId)}
                      />
                      <datalist id="product-candidates">
                        {suggestedProducts.map((item) => (
                          <option key={item.id} value={`${item.styleNumber} - ${item.name}`}>
                            {item.division?.name || ''} / {item.category || ''}
                          </option>
                        ))}
                      </datalist>
                    </div>
                    <div className="bp-form-group">
                      ...
                      <input type="date" className="bp-input" value={salesStart} onChange={(e) => setSalesStart(e.target.value)} />
                    </div>
                    <div className="bp-form-group">
                      <label className="bp-label">PO Deadline</label>
                      <input type="date" className="bp-input" value={poDeadline} onChange={(e) => setPoDeadline(e.target.value)} />
                    </div>
                    <div className="bp-form-group">
                      <label className="bp-label">Original Price</label>
                      <input type="number" className="bp-input" value={originalPrice} onChange={(e) => setOriginalPrice(e.target.value)} placeholder="e.g. 5000" />
                    </div>
                    <div className="bp-form-group">
                      <label className="bp-label">Plan Qty</label>
                      <input type="number" className="bp-input" value={planQty} onChange={(e) => setPlanQty(e.target.value)} placeholder="e.g. 100" />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bp-card" style={{ marginBottom: 24 }}>
            <div className="bp-card__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Factory style={{ width: 18, height: 18, color: 'var(--color-primary)' }} />
                <h3 style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>{t('productionInfo')}</h3>
              </div>
            </div>
            <div className="bp-card__content">
                ...
                <div className="bp-form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (matched) {
                        setMainFactoryCode(matched.factoryCode || '')
                        setOriginCountry(matched.country || '')
                      }
                    }}
                    placeholder="Search supplier..."
                  />
                  <datalist id="supplier-lookup-list">
                    {suppliers
                      .filter((s) => !supplierInput || s.name.toLowerCase().includes(supplierInput.toLowerCase()))
                      .slice(0, 30)
          ...
          <div className="bp-card" style={{ marginBottom: 24 }}>
            <div className="bp-card__header">
              ...
              <button type="button" className="bp-button bp-button--secondary bp-button--sm" onClick={() => setColorRows((prev) => [...prev, { colorId: '', colorInput: '', status: 'IN_PROGRESS' }])}>
                      ...
                      return (
                        <tr key={`color-row-${index}`}>
                          <td>
                            <input
                              className="bp-input"
                              list={`color-lookup-list-${index}`}
                              value={row.colorInput}
                              onChange={(e) => {
                                const val = e.target.value
                                const matched = colors.find((c) => `${c.colorCode} - ${c.colorName}` === val)
                                setColorRows((prev) => prev.map((x, i) => (i === index ? { ...x, colorInput: val, colorId: matched?.id || '' } : x)))
                              }}
                              placeholder="Search color..."
                            />
                            <datalist id={`color-lookup-list-${index}`}>
                              {colors
                                .filter((c) => !row.colorInput || `${c.colorCode} ${c.colorName}`.toLowerCase().includes(row.colorInput.toLowerCase()))
                                .slice(0, 30)
                                .map((c) => <option key={c.id} value={`${c.colorCode} - ${c.colorName}`} />)}
                            </datalist>
                          </td>
                          <td style={{ padding: 2, verticalAlign: 'middle', textAlign: 'center' }}>
                            {color?.colorImage
                              ? <img src={color.colorImage} alt={color.colorCode} style={{ width: 33, height: 33, objectFit: 'cover', borderRadius: 4, border: '1px solid var(--color-border)', display: 'block', margin: '0 auto' }} />
                              : <div style={{ width: 33, height: 33, border: '1px solid var(--color-border)', borderRadius: 4, margin: '0 auto' }} />}
                          </td>
                          <td>
          ...
          <div className="bp-card" style={{ marginBottom: 24 }}>
            ...
            <div className="bp-card__content">
              ...
              <div className="bp-form-group" style={{ marginTop: 16 }}>
                ...
                {clo3dPreviewUrl && <Clo3dViewer fileDataUrl={clo3dPreviewUrl} fileName={clo3dFileName} height={400} />}
              </div>
            </div>
          </div>
          ...
          <div className="bp-card" style={{ marginBottom: 24 }}>
            <div className="bp-card__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Package style={{ width: 20, height: 20, color: 'var(--color-primary)' }} />
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <input
                          className="bp-input"
                          list="bom-main-fabric-code-list"
                          value={mainFabric.materialCode}
                          onChange={(e) => {
                            const val = e.target.value
                            ...
                            if (picked) {
                              ...
                              setMainFabric((prev) => ({ ...prev, materialCode: val, materialMasterId: '' }))
                            }
                          }}
                          placeholder="Search..."
                        />
                        <datalist id="bom-main-fabric-code-list">
                          {filterMaterials(mainFabricMasters, mainFabric.materialCode).map((m) => (
                            <option key={m.id} value={m.name} />
                          ))}
                        </datalist>
                      </td>
                      <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{mainFabric.materialName || '-'}</td>
                      <td><input className="bp-input" value={mainFabric.placement || ''} onChange={(e) => setMainFabric({ ...mainFabric, placement: e.target.value })} placeholder="Body, Lining..." /></td>
                      <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{mainFabric.costPerUnit || '-'}</td>
                      ...
                      <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{mainFabric.fabricSupplier || '-'}</td>
                      <td />
                    </tr>
                    ...
                    <tr>
                      <td colSpan={7} style={{ background: 'var(--color-gray-50)', fontWeight: 700 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <button type="button" className="bp-button bp-button--secondary bp-button--sm" onClick={() => setSubFabrics((prev) => [...prev, { materialMasterId: '', materialCode: '', materialName: '', placement: '', costPerUnit: '', fabricSupplier: '', unit: '' }])}>
                            <Plus style={{ width: 14, height: 14 }} />
                            {tCommon('new')}
                          </button>
                        </div>
                    ...
                    {subFabrics.map((item, index) => (
                      <tr key={`sf-${index}`}>
                        <td>
                          <input
                            className="bp-input"
                            list={`bom-sub-fabric-code-${index}`}
                            ...
                            onChange={(e) => {
                              ...
                              if (picked) {
                                ...
                                setSubFabrics((prev) => prev.map((x, i) => (i === index ? { ...x, materialCode: val, materialMasterId: '' } : x)))
                              }
                            }}
                            placeholder="Search..."
                          />
                          <datalist id={`bom-sub-fabric-code-${index}`}>
                            {filterMaterials(subFabricMasters, item.materialCode).map((m) => (
                              <option key={m.id} value={m.name} />
                            ))}
                          </datalist>
                        </td>
                        <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{item.materialName || '-'}</td>
                        <td><input className="bp-input" value={item.placement || ''} onChange={(e) => setSubFabrics((prev) => prev.map((x, i) => (i === index ? { ...x, placement: e.target.value } : x)))} placeholder="Collar, Cuff..." /></td>
                        <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{item.costPerUnit || '-'}</td>
                        ...
                        <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{item.fabricSupplier || '-'}</td>
                        <td>
                    ...
                    <tr>
                      <td colSpan={7} style={{ background: 'var(--color-gray-50)', fontWeight: 700 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <button type="button" className="bp-button bp-button--secondary bp-button--sm" onClick={() => setSubMaterials((prev) => [...prev, { materialMasterId: '', materialCode: '', materialName: '', placement: '', costPerUnit: '', fabricSupplier: '', unit: '' }])}>
                            <Plus style={{ width: 14, height: 14 }} />
                            {tCommon('new')}
                          </button>
                        </div>
                    ...
                    {subMaterials.map((item, index) => (
                      <tr key={`sm-${index}`}>
                        <td>
                          <input
                            className="bp-input"
                            list={`bom-sub-material-code-${index}`}
                            value={item.materialCode}
                            onChange={(e) => {
                              const val = e.target.value
                              ...
                              if (picked) {
                                ...
                                setSubMaterials((prev) => prev.map((x, i) => (i === index ? { ...x, materialCode: val, materialMasterId: '' } : x)))
                              }
                            }}
                            placeholder="Search..."
                          />
                          <datalist id={`bom-sub-material-code-${index}`}>
                            {filterMaterials(subMaterialMasters, item.materialCode).map((m) => (
                              <option key={m.id} value={m.name} />
                            ))}
                          </datalist>
                        </td>
                        <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{item.materialName || '-'}</td>
                        <td><input className="bp-input" value={item.placement || ''} onChange={(e) => setSubMaterials((prev) => prev.map((x, i) => (i === index ? { ...x, placement: e.target.value } : x)))} placeholder="Button, Zipper..." /></td>
                        <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{item.costPerUnit || '-'}</td>
          ...
          <div className="bp-card" style={{ marginBottom: 24 }}>
            <div className="bp-card__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <DollarSign style={{ width: 20, height: 20, color: 'var(--color-primary)' }} />
            ...
            <div className="bp-card__content">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
                ...
                <div className="bp-form-group">
                  ...
                  <input type="date" lang="en" className="bp-input" value={fcrDate} onChange={(e) => setFcrDate(e.target.value)} tabIndex={0} />
                </div>
                <div className="bp-form-group">
                  <label className="bp-label">PO Deadline</label>
                  <input type="date" lang="en" className="bp-input" value={poDeadline} onChange={(e) => setPoDeadline(e.target.value)} tabIndex={0} />
                </div>
              </div>
                ...
                <table className="bp-table" style={{ fontSize: 13 }}>
                  ...
                  <tbody>
                    ...
                    <tr style={{ backgroundColor: 'var(--color-gray-50)', fontWeight: 600 }}>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        {(() => {
                          const mainCost = mainFabric.costPerUnit ? parseFloat(mainFabric.costPerUnit) : 0
              ...
              {commentsCollapsed ? (
                ...
                <>
                  <div className="bp-card__header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 className="bp-card__title">Communication History</h2>
                    <button
                      type="button"
        ...
        {/* End Communication History sidebar */}
      </div>
    </form>
  )
}
