...
export async function createSampleWithNormalizedData(tx: Prisma.TransactionClient, productId: string | null | undefined, body: any) {
  ...
  const sample = await tx.sample.create({
    data: {
      ...
      salesStart: sampleInfo.salesStart ? new Date(sampleInfo.salesStart) : null,
      poDeadline: sampleInfo.poDeadline ? new Date(sampleInfo.poDeadline) : null,
      originalPrice: sampleInfo.originalPrice ? parseFloat(sampleInfo.originalPrice) : null,
      planQty: sampleInfo.planQty ? parseInt(sampleInfo.planQty) : null,
